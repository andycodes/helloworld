...
static int __devinit buttons_probe(struct platform_device *pdev)
{
	int ret;
	struct resource *r;
...
static int __devexit buttons_remove(struct platform_device *pdev)
{
	return 0;
}

static void buttons_shutdown(struct platform_device *pdev)
{
    return;
}
...
MODULE_DESCRIPTION("Driver for the buttons");
MODULE_LICENSE("GPL");

/*
实验目的：启用MMU，映射SDRAM的地址空间，
操作虚拟地址实现“点灯大法”，借此掌握MMU的使用。

实验环境及说明：恒颐S3C2410开发板H2410。H2410核心板
扩展有64MB的K4S561632 SDRAM(4M*16bit*4BANK)，地址范围是0x30000000~0x33FFFFFF。
GPIO端口的地址范围是0x56000000~0X560000B0。

实验思路：开发板上电启动后，自动将NandFlash开始的4K数据
复制到SRAM中，然后跳转到0地址开始执行，然后初始化存储
控制器SDRAM，把2K后的代码从SRAM中复制到SDRAM中(存放在0x30004000，
前16KB用来存放页表)、设置页表、启动MMU实现虚拟地址映射GPIO寄存
器和SDRAM，最后跳转到SDRAM中(地址0xB0004000)运行。重新设置栈指针，
跳到点灯代码的入口点实现点灯操作
*/

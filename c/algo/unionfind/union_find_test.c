...
3 <-- --> 1
     ...
     v
     2
这样编号的好处是什么呢？可以发现，只有在这
些情况下，两个相邻的单元格才能相连：
如果某一个单元格有第 00 个方向，那么它上方的
单元格必须有第 22 个方向；
如果某一个单元格有第 11 个方向，那么它右侧的
单元格必须有第 33 个方向；
如果某一个单元格有第 22 个方向，那么它下方
的单元格必须有第 00 个方向；
如果某一个单元格有第 33 个方向，那么它左侧
的单元格必须有第 11 个方向。
发现了什么规律？
如果某一个单元格有第 ii 个方向，那么它在第 
ii 个方向相邻的
单元格必须有第 (i + 2) \% 4(i+2)%4 个方向。
因此，我们遍历整个网格，对于其中的每一个
单元格 (x, y)(x,y)，
我们枚举它的四个相邻的单元格，对于第 ii 个
方向的相邻单元
格，它和 (x, y)(x,y) 相连当且仅当 (x, y)(x,y) 对应的
二进制数的第 ii 位
和相邻单元格对应的二进制数的第 (i + 2) % 4 位
均为 11。
在遍历结束之后，我们判断左上角和右下角
在并查集中是否
属于同一集合即可。

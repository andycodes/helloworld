...
int hook_fopen(){
/*
获取目标pid进程中"/data/app-lib/com.bbk.appstore-2/libvivosgmain.so"模块
的加载地址
*/
    ...
    Elf32_Ehdr ehdr;
/*
读取elf32格式的文件"/data/app-lib/com.bbk.appstore-2/libvivosgmain.so"
的文件头信息
*/
...
/*
elf32文件节区表中每个节区的名称存放的节区名称
字符串表，在节区表中的序号index
*/
    unsigned long stridx = ehdr.e_shstrndx;
/*
elf32文件中节区表的每个单元信息结构体（描述每个节
区的信息的结构体）
*/
    Elf32_Shdr shdr;
/*
elf32文件中定位到存放每个节区名称的字符串表的
信息结构体位置.shstrtab
*/
    lseek(fd, shdr_addr + stridx * shent_size, SEEK_SET);
/*
读取elf32文件中的描述每个节区的信息的结构体
（这里是保存elf32文件的每个节区的名称字符串的）
*/
    read(fd, &shdr, shent_size);
    LOGD("[+] String table offset is %lu, size is %lu", shdr.sh_offset, shdr.sh_size); //41159, size is 254
/*
// 为保存elf32文件的所有的节区的名称字符串申请
内存空间
*/
    char * string_table = (char *)malloc(shdr.sh_size);
/*
    // 定位到具体存放elf32文件的所有的节区的
    名称字符串的文件偏移处
*/
